...
<body>
<form id="form1" name="form1" method="post" action="editInfos">
  <security:csrfInput/>
<input name="i_id" type="hidden" value="${info.i_id}"/>
  <p>标　　题：
    <label for=""></label>
    <input name="title" type="text" id="title" value="${info.title}"/>
    <span style="color:red">${map.msg}</span>  
  </p>
  <p>作　　者：
    <label for="author"></label>
    <input type="text" name="author" id="author" value="${info.author}"/>
  </p>
  <p>发布日期：
    <label for="adddate"></label>
    <input type="text" name="adddate" id="adddate" value="${info.adddate}" />
  </p>
  <p>内　　容：</p>
  <p>	
    <label for="content"></label>
    <textarea name="content" id="myEditor" value="${info.content}">${info.content }</textarea> 
  </p>
  <p>大栏目：
    <label for="maxid"></label>
    <select name="maxid" id="maxid">
      <option value="${info.maxid}">${maxname }</option>
  ...
  <select name="minid" id="minid">
    <option value="${info.minid}">${minname }</option>
  </select>
  </p>
  <p>
